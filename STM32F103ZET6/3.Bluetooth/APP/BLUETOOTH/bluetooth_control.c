...
void Bluetooth_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	/* Configure USART3 Tx  as alternate function push-pull 推拉输出模式*/
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);    
	/* Configure USART3 Rx  as input floating 浮点输入模式*/
...
void Protocol(void)
{
	...
	if(strlen((const char *)Data_to_String)<21)
	{
		...
		//USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);//使能接收中断
		USART3_Send_Char("$ReceivePackError#"); //返回协议数据包 
		return;
	}
	...
	{
		State_Flag = 5;
		USART3_Send_Char(returntemp); //返回协议数据包  	
